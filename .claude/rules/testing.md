...
- A behaviour change ships with a test that would fail without it. Engine
  ...
  `crates/storage/tests/branching.rs` (both backends), the scale-to-zero
  lifecycle in `crates/controller/src/tests.rs`, and the server-mode (pgwire)
  benchmark driver in `crates/bench/tests/pgwire.rs` (conflict classification +
  no-lost-update over the wire, against an in-process listener).
